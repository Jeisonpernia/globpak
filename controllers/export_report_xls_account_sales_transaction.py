...
class ExportReportXlsAccountSalesTransaction(http.Controller):

    @http.route('/web/export_xls/sales_transaction', type='http', auth="user")
    def export_xls(self, filename, title, subtitle, company_id, date_from, date_to, account_id, **kw):
        ...
        account_invoice_sales = request.env['account.invoice'].search([('type', '=', 'out_invoice'),('state','=','paid'),('date','>=',date_from),('date','<=',date_to)])
        ...
        worksheet.write(8, 3, 'NAME OF CUSTOMER (Last Name, First Name, Middle Name)', style_table_header_bold) # HEADER
        worksheet.write(8, 4, "CUSTOMER'S ADDRESS", style_table_header_bold) # HEADER
        
        worksheet.write(8, 5, 'AMOUNT OF GROSS SALES', style_table_header_bold) # HEADER
        worksheet.write(8, 6, 'AMOUNT OF EXEMPT SALES', style_table_header_bold) # HEADER
        worksheet.write(8, 7, 'AMOUNT OF ZERO RATED SALES', style_table_header_bold) # HEADER
        worksheet.write(8, 8, 'AMOUNT OF TAXABLE SALES', style_table_header_bold) # HEADER
        worksheet.write(8, 9, 'AMOUNT OF OUTPUT TAX', style_table_header_bold) # HEADER
        worksheet.write(8, 10, 'AMOUNT OF GROSS TAXABLE SALES', style_table_header_bold) # HEADER
        ...
        for account in account_invoice_sales:
            ...
            worksheet.write(row_count, 9, account.amount_tax, style_table_row_amount)
            worksheet.write(row_count, 10, gross_taxable_sales, style_table_row_amount)
        ...
        worksheet.write(table_total_start, 9, total_amount_tax, style_table_total_value)
        worksheet.write(table_total_start, 10, total_gross_taxable_sales, style_table_total_value)
