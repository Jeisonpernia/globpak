...
class ExportReportXlsAccountPurchaseTransaction(http.Controller):
    ...
    def export_xls(self, filename, title, subtitle, company_id, date_from, date_to, journal_id, **kw):
        ...
        account_invoice_purchase = request.env['account.invoice'].search([('journal_id.id', '=', journal_id),('is_purchase','=',True),('type','=','in_invoice'),('state','in',('open','paid')),('date','>=',date_from),('date','<=',date_to)],order='date asc')
