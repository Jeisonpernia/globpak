...
class AccountReportImportTransaction(models.TransientModel):
    ...
    _description = 'Import Transaction Report'

    def _default_account_id(self):
        account_id = self.env['account.account'].search([('name','=','Account Payable')], limit=1)
        return account_id
    ...
    def _print_report(self, data):
        ...
        return {
            ...
            'url': '/web/export_xls/import_transaction?filename=%s&title=%s&subtitle=%s&company_id=%s&date_from=%s&date_to=%s&account_id=%s'%(filename,title,subtitle,company_id,date_from,date_to,account_id),
            'target': 'self',
        }                     
