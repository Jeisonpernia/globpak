...
class AccountReportAlphalistPayee(models.TransientModel):
    ...
    def _default_journal_id(self):
        journal_id = self.env['account.account'].search([('name','=','Withholding Tax Expanded')], limit=1)
        return journal_id
    ...
    def _print_report(self, data):
        filename = 'account_alphalist_payee_report.xls'
        ...
        return {
            ...
            'url': '/web/export_xls/account_alphalist_payee?filename=%s&title=%s&company_id=%s&date_from=%s&date_to=%s&account_id=%s'%(filename,title,company_id,date_from,date_to,account_id),
            'target': 'self',
        }                     
